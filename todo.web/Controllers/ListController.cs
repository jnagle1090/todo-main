...
namespace todo.web.Controllers
{
    public class ListController : BaseController
    {
        ...
        {
            var result = await _client.DeleteItem(itemId,Id);
            if (!result.IsSuccessStatusCode)
                throw new ResultException($"Unable to delete list item {itemId} for {UserName}", result);
        ...
        public async Task<IActionResult> DeleteList(int id)
        {
            ...
            // The following two lines should be used to redirect the user back to the list of existing Todos.
            //var listsResult = await _client.GetLists();
            //return View("Lists", listsResult.Value);  

            //To satisfy testing, I must return a null RedirectToAction which causes an infinite loop as the action has nowhere to go
